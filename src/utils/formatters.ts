...
export function formatClosedOrders(orders: ClosedOrder[]): string {
  ...
  // Calculate total P&L
  const totalPnL = orders.reduce((sum: number, o: ClosedOrder) => sum + o.final_pnl, 0);
  const profitable = orders.filter((o: ClosedOrder) => o.final_pnl > 0).length;
...
export function formatDashboard(data: DashboardData): string {
  const totalPnL = data.closed_orders.reduce((sum: number, o: ClosedOrder) => sum + o.final_pnl, 0);
  const profitable = data.closed_orders.filter((o: ClosedOrder) => o.final_pnl > 0).length;
